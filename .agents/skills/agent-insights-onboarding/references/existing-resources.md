...
- Existing Agent, or Prompt/Hosted sample Agent creation
...
- When the user selects a new sample Agent, create it in the selected project with a
  deterministic run-derived name and ownership metadata. Generate the bounded sample
...
- Explicit cleanup preserves every pre-existing Agent, monitor, and project identity. For
  ...
  removes only connection and role resources whose live principal, role, scope, target,
  and deterministic name still match the frozen plan.
